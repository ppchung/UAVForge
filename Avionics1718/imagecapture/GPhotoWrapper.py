...
def summary():
  y = subprocess.Popen(['sudo', 'gphoto2', '--summary'], stdout=subprocess.PIPE)
  out, err = y.communicate()
  print(out)
  return out

def lapse(fname,interval,count):
  x = subprocess.Popen(['sudo', 'gphoto2', '-I '+interval, '-F '+count, '--capture-image-and-download', '--filename=%Y%m%d%H%M%S'+fname+'.jpg'], stdout=subprocess.PIPE)
